...
var permKeyCmd = &cobra.Command{
	...
	qV2Cli depl permKey [keyFile] [password] [adminOrg] [newAdminRole] [orgAdminRole] [subOrgDepth] [subOrgBreadth]`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if pass == "" {
			err := deployKey(keyFile, "", adminOrg, newAdminRole, orgAdminRole, subOrgDepth, subOrgBreadth)
			if err != nil {
				return err
			}
		} else {
			err := deployKey(keyFile, pass, adminOrg, newAdminRole, orgAdminRole, subOrgDepth, subOrgBreadth)
			if err != nil {
				return err
			}
		}
...
func init() {
	...
	deplCmd.AddCommand(permKeyCmd)

}

func deployKey(keyFile, password, adminOrg, newAdminRole, orgAdminRole string, subOrgDepth, subOrgBreadth int) error {
	...
	fmt.Println("the guardian address is :", add)
